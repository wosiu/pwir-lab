...
__global__ void doReductionCuda(float *A, float B*)
{
...
int main()
{
    ...
    dotProductCuda<<<(VECTOR_DIM + BLOCK_SIZE - 1)/BLOCK_SIZE, BLOCK_SIZE>>>(d_C, d_A, d_B);
    
    cudaCheck(cudaPeekAtLastError());
    
    double result = 0;
    int pre_red_size = (VECTOR_DIM + BLOCK_SIZE - 1)/BLOCK_SIZE;
    int red_block_num = (pre_red_size + BLOCK_SIZE - 1)/BLOCK_SIZE;

    // reductions
    float *h_D = (float *)malloc(red_block_num);
    float *d_D;
 
    cudaCheck(cudaMalloc((void **) &d_D, red_block_num));
   
    doReductionCuda<<<(red_block_num, BLOCK_SIZE>>>(d_C, d_D);

    cudaCheck(cudaMemcpy(d_D, h_D, vector_mem_size, cudaMemcpyDeviceToHost));

    for (int i = 0; i < red_block_num; i++) {
	result += h_D[i];
    }
